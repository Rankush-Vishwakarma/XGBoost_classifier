...
def loadModel():
    try:
        with open(r"xgb_classifier_new.pkl", "rb") as XGB_input:
            XGB = pickle.load(XGB_input)
        return XGB
...
def predict(data):
    try:
        model = {}
        # we have to normalize the numerical data first
        #data = [float(i) for i in data]
        new_data = np.array(data[:3]).reshape(-1,1)
        scaler = MinMaxScaler()
        normalized_data = scaler.fit_transform(new_data).reshape(-1)
        print(normalized_data)
        data = list(normalized_data) + data[3:] # this will become our new data points
        d = np.array(data).reshape(1,-1)
        print(d)
        XGB = loadModel()
        model['XGBoost'] = XGB.predict(d)
            ...
            if request.method == 'POST':
                ...
                def dict2htmltable(data):
                    try:
                        ...
                        if class_ == 0:
                            result  = 'Wage Class less than or equal to 50k';
                        if class_ ==1:
                            result  = 'Wage Class greater than 50K';
